...
export async function GET(request: NextRequest) {
  try {
    ...
    // Approve certificate
    await certificatesService.approve(certificateId, "Email Approval System")

    // Redirect to success page
    return NextResponse.redirect(new URL(`/api/email-approval/status?status=approved&cert=${certificate.certificateNo}`, request.url))
  } catch (error) {
