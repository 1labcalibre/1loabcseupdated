...
export async function GET(request: NextRequest) {
  try {
    ...
    if (certificate.status === 'rejected') {
      return new NextResponse(
        generateAlreadyProcessedPage('Already Rejected', 'This certificate has already been rejected. No further action is required.', certificate, 'rejected'),
    ...
    if (certificate.status === 'approved') {
      return new NextResponse(
        generateAlreadyProcessedPage('Already Approved', 'This certificate has already been approved. The rejection action cannot be completed.', certificate, 'approved'),
        { status: 200, headers: { 'Content-Type': 'text/html' } }
      )
    }

    // Update certificate status - store this action for processing
    console.log(`Certificate ${certificateId} rejected via email`)
    
    // Store the rejection action in our token storage for the certificate to be updated
    tokenData.certificateData.status = 'rejected'
    tokenData.certificateData.rejectedAt = new Date().toISOString()
    tokenData.certificateData.rejectedBy = 'Email Approval'
    tokenData.certificateData.rejectedVia = 'email'
    ...
    console.log(`Certificate ${certificateId} marked for rejection, will sync on next app visit`)
    ...
    return new NextResponse(
      ...
      <head>
          ...
          <style>
              ...
              .cross-mark { font-size: 48px; color: #f44336; margin-bottom: 20px; }
              .next-steps {
                  background: #fff3cd;
      ...
      <body>
          ...
          <h1>Certificate Rejected</h1>
          <div class="rejected">
              <h3>Rejection Confirmed</h3>
              <p>The certificate has been rejected and marked for revision.</p>
          </div>
          ...
          <div class="next-steps">
              <h4>Next Steps:</h4>
              <p>The certificate creator will be notified of this rejection. They can make necessary changes and resubmit for approval.</p>
          </div>
          <p>
              <a href="/login" class="btn">Go to Application</a>
              <a href="/certificates" class="btn btn-edit">Edit Certificate</a>
          </p>
          <p style="color: #666; font-size: 14px; margin-top: 30px;">
              This window can be safely closed. The rejection has been recorded in the system.
          </p>
      </body>
      </html>
      `,
    ...
    return new NextResponse(
      generateErrorPage('Error Processing Request', 'An error occurred while processing your rejection. Please try again or contact support.'),
